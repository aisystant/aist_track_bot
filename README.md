...
| `/update` | Изменить настройки (язык, сложность) |
...
1. **Планирование недели** — бот предлагает до 5 тем на основе ваших интересов
2. **Выбор тем** — выберите те, что откликаются (можно предложить свою тему)
3. **Ежедневные дайджесты** — получайте материал каждый день
...
| Марафон | Лента |
|---------|-------|
| Фиксированная программа | Темы на выбор |
| 14 дней подряд | Любая длительность |
| Уроки + задания | Только дайджесты |
...
3. **Лента** — переключитесь на `/feed`, выберите темы, получите дайджест
4. **Консультация** — задайте вопрос в свободной форме (например, «Что такое системное мышление?»)
5. **Смена языка** — проверьте через `/update`, что интерфейс переключается
6. **Переключение режимов** — убедитесь, что переход между Марафоном и Лентой работает
