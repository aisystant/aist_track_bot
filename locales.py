...
TRANSLATIONS = {
    'ru': {
        ...
        'progress.active_days_week': 'Активных дней за неделю (Марафон+Лента)',
        'progress.marathon_title': 'Марафон',
        'progress.day_of_total': 'День {day}/{total}',
        'progress.topics_done': 'Пройдено тем',
        'progress.work_products': 'Рабочих продуктов',
        'progress.feed_title': 'Лента',
        'progress.digests': 'Дайджестов',
        'progress.fixations': 'Фиксаций',
        'progress.topics': 'Темы',
        'progress.topics_not_selected': 'не выбраны',
        'progress.full_report': 'Полный отчёт',
        'progress.first_start': 'Сначала /start',

        # Режимы
        'modes.select': 'Выбери режим',
        'modes.marathon_desc': '14-дневный марафон',
        'modes.select_title': 'Выберите режим обучения',
        'modes.current_mode': 'Текущий режим',
        'modes.marathon_name': 'Марафон',
        'modes.marathon_14day': '14-дневный курс',
        'modes.marathon_daily_desc': 'Каждый день изучаете две темы: теория + практика',
        'modes.feed_name': 'Лента',
        'modes.feed_infinite': 'бесконечный режим',
        'modes.feed_ai_desc': 'ИИ предлагает темы, вы выбираете что изучать',
        'modes.both_name': 'Оба режима',
        'modes.marathon_activated': 'Режим Марафон активирован!',
        'modes.feed_activated': 'Режим Лента активирован!',
        'modes.your_settings': 'Ваши настройки',
        'modes.your_topics': 'Ваши темы',
        'modes.time': 'Время',
        'modes.reading_time': 'На чтение',
        'modes.minutes': 'мин',
        'modes.complexity': 'Сложность',
        'modes.complexity_beginner': 'Начальный',
        'modes.complexity_basic': 'Базовый',
        'modes.complexity_medium': 'Средний',
        'modes.complexity_advanced': 'Продвинутый',
        'modes.complexity_expert': 'Экспертный',
        'modes.additional_reminder': 'Доп.напоминание',
        'modes.feed_paused': 'Лента на паузе. Вернуться: /mode',
        'modes.marathon_paused': 'Марафон на паузе. Вернуться: /mode',
        # Статусы режимов
        'modes.status_active': 'Активен',
        'modes.status_active_day': 'Активен (день {day}/{total}, пройдено {done} тем)',
        'modes.status_paused': 'На паузе',
        'modes.status_paused_day': 'На паузе (день {day}/{total})',
        'modes.status_not_started': 'Не начат',
        'modes.status_not_started_f': 'Не начата',
        'modes.status_completed': 'Завершён',
        'modes.status_unknown': 'Статус неизвестен',
        'modes.status_active_days': 'активных дней: {days}',
        # Дополнительные строки для режимов
        'modes.error_occurred': 'Произошла ошибка. Попробуйте ещё раз.',
        'modes.use_learn': 'Используйте /learn для продолжения обучения.',
        'modes.start_date': 'Дата старта',
        'modes.start_date_not_set': 'не установлена',
        'modes.current': 'Текущая',
        'modes.start_date_set': 'Дата старта: {date}',
        'modes.reset_marathon_title': 'Сбросить марафон?',
        'modes.will_be_reset': 'Будет сброшено',
        'modes.topics_passed': 'пройденных тем',
        'modes.all_progress': 'Весь прогресс по дням',
        'modes.feed_stats_kept': 'Статистика Ленты сохранится.',
        'modes.yes_reset': 'Да, сбросить',
        'modes.cancel': 'Отмена',
        'modes.marathon_reset': 'Марафон сброшен!',
        'modes.new_start_date': 'Новая дата старта',
        'modes.use_learn_start': 'Используйте /learn для начала обучения.',
        'modes.reminders_title': 'Напоминания',
        'modes.current_time': 'Сейчас',
        'modes.enter_time_format': 'Введите время в формате ЧЧ:ММ',
        'modes.time_example': 'Например: `07:30` или `18:00`',
        'modes.two_reminders_hint': 'Для двух напоминаний введите через запятую:',
        'modes.two_reminders_example': 'Например: `07:00, 19:00`',
        'modes.invalid_time_format': 'Неверный формат времени.',
        'modes.enter_time_example': 'Введите в формате ЧЧ:ММ, например: `09:00` или `07:30, 19:00`',
        'modes.second_reminder_removed': 'Второе напоминание удалено',
        'modes.time_set': 'Время установлено: {time}',
        'modes.reminder_set': 'Напоминание установлено: {time}',
        'modes.reminders_set': 'Напоминания установлены: {time1}, {time2}',
        'modes.complexity_title': 'Сложность вопросов',
        'modes.complexity_set': 'Сложность: {level}',
        'modes.level_basic': 'Базовый',
        'modes.level_basic_desc': 'понимание основ',
        'modes.level_medium': 'Средний',
        'modes.level_medium_desc': 'применение на практике',
        'modes.level_advanced': 'Продвинутый',
        'modes.level_advanced_desc': 'анализ и синтез',
        'modes.change_time': 'Изменить время',
        'modes.second': 'Второе',
        'modes.add_second': 'Добавить второе',
        'modes.select_time': 'Выберите время',
        'modes.tomorrow': 'Завтра',
        'modes.day_after_tomorrow': 'Послезавтра',
    ...
    'en': {
        ...
        'buttons.update_data': 'Update data',
        'buttons.reminders': 'Reminders',
        'buttons.reset_marathon': 'Reset marathon',
        'buttons.back': '« Back',
        'buttons.settings': 'Settings',
        'buttons.get_digest_full': 'Get digest',
        ...
        'progress.active_days_week': 'Active days this week (Marathon+Feed)',
        'progress.marathon_title': 'Marathon',
        'progress.day_of_total': 'Day {day}/{total}',
        'progress.topics_done': 'Topics completed',
        'progress.work_products': 'Work products',
        'progress.feed_title': 'Feed',
        'progress.digests': 'Digests',
        'progress.fixations': 'Fixations',
        'progress.topics': 'Topics',
        'progress.topics_not_selected': 'not selected',
        'progress.full_report': 'Full report',
        'progress.first_start': 'First /start',

        # Modes
        'modes.select': 'Select mode',
        'modes.marathon_desc': '14-day marathon',
        'modes.select_title': 'Select learning mode',
        'modes.current_mode': 'Current mode',
        'modes.marathon_name': 'Marathon',
        'modes.marathon_14day': '14-day course',
        'modes.marathon_daily_desc': 'Each day you study two topics: theory + practice',
        'modes.feed_name': 'Feed',
        'modes.feed_infinite': 'infinite mode',
        'modes.feed_ai_desc': 'AI suggests topics, you choose what to study',
        'modes.both_name': 'Both modes',
        'modes.marathon_activated': 'Marathon mode activated!',
        'modes.feed_activated': 'Feed mode activated!',
        'modes.your_settings': 'Your settings',
        'modes.your_topics': 'Your topics',
        'modes.time': 'Time',
        'modes.reading_time': 'Reading time',
        'modes.minutes': 'min',
        'modes.complexity': 'Complexity',
        'modes.complexity_beginner': 'Beginner',
        'modes.complexity_basic': 'Basic',
        'modes.complexity_medium': 'Medium',
        'modes.complexity_advanced': 'Advanced',
        'modes.complexity_expert': 'Expert',
        'modes.additional_reminder': 'Additional reminder',
        'modes.feed_paused': 'Feed paused. Return: /mode',
        'modes.marathon_paused': 'Marathon paused. Return: /mode',
        # Mode statuses
        'modes.status_active': 'Active',
        'modes.status_active_day': 'Active (day {day}/{total}, {done} topics done)',
        'modes.status_paused': 'Paused',
        'modes.status_paused_day': 'Paused (day {day}/{total})',
        'modes.status_not_started': 'Not started',
        'modes.status_not_started_f': 'Not started',
        'modes.status_completed': 'Completed',
        'modes.status_unknown': 'Status unknown',
        'modes.status_active_days': 'active days: {days}',
        # Additional mode strings
        'modes.error_occurred': 'An error occurred. Please try again.',
        'modes.use_learn': 'Use /learn to continue learning.',
        'modes.start_date': 'Start date',
        'modes.start_date_not_set': 'not set',
        'modes.current': 'Current',
        'modes.start_date_set': 'Start date: {date}',
        'modes.reset_marathon_title': 'Reset marathon?',
        'modes.will_be_reset': 'Will be reset',
        'modes.topics_passed': 'topics completed',
        'modes.all_progress': 'All day progress',
        'modes.feed_stats_kept': 'Feed statistics will be kept.',
        'modes.yes_reset': 'Yes, reset',
        'modes.cancel': 'Cancel',
        'modes.marathon_reset': 'Marathon reset!',
        'modes.new_start_date': 'New start date',
        'modes.use_learn_start': 'Use /learn to start learning.',
        'modes.reminders_title': 'Reminders',
        'modes.current_time': 'Current',
        'modes.enter_time_format': 'Enter time in HH:MM format',
        'modes.time_example': 'Example: `07:30` or `18:00`',
        'modes.two_reminders_hint': 'For two reminders, separate with comma:',
        'modes.two_reminders_example': 'Example: `07:00, 19:00`',
        'modes.invalid_time_format': 'Invalid time format.',
        'modes.enter_time_example': 'Enter in HH:MM format, example: `09:00` or `07:30, 19:00`',
        'modes.second_reminder_removed': 'Second reminder removed',
        'modes.time_set': 'Time set: {time}',
        'modes.reminder_set': 'Reminder set: {time}',
        'modes.reminders_set': 'Reminders set: {time1}, {time2}',
        'modes.complexity_title': 'Question complexity',
        'modes.complexity_set': 'Complexity: {level}',
        'modes.level_basic': 'Basic',
        'modes.level_basic_desc': 'understanding basics',
        'modes.level_medium': 'Medium',
        'modes.level_medium_desc': 'practical application',
        'modes.level_advanced': 'Advanced',
        'modes.level_advanced_desc': 'analysis and synthesis',
        'modes.change_time': 'Change time',
        'modes.second': 'Second',
        'modes.add_second': 'Add second',
        'modes.select_time': 'Select time',
        'modes.tomorrow': 'Tomorrow',
        'modes.day_after_tomorrow': 'Day after tomorrow',
    ...
    'es': {
        ...
        'buttons.update_data': 'Actualizar datos',
        'buttons.reminders': 'Recordatorios',
        'buttons.reset_marathon': 'Reiniciar maratón',
        'buttons.back': '« Volver',
        'buttons.settings': 'Ajustes',
        'buttons.get_digest_full': 'Obtener resumen',
        ...
        'progress.active_days_week': 'Días activos esta semana (Maratón+Feed)',
        'progress.marathon_title': 'Maratón',
        'progress.day_of_total': 'Día {day}/{total}',
        'progress.topics_done': 'Temas completados',
        'progress.work_products': 'Productos de trabajo',
        'progress.feed_title': 'Feed',
        'progress.digests': 'Resúmenes',
        'progress.fixations': 'Fijaciones',
        'progress.topics': 'Temas',
        'progress.topics_not_selected': 'no seleccionados',
        'progress.full_report': 'Informe completo',
        'progress.first_start': 'Primero /start',

        # Modos
        'modes.select': 'Seleccionar modo',
        'modes.marathon_desc': 'Maratón de 14 días',
        'modes.select_title': 'Selecciona el modo de aprendizaje',
        'modes.current_mode': 'Modo actual',
        'modes.marathon_name': 'Maratón',
        'modes.marathon_14day': 'Curso de 14 días',
        'modes.marathon_daily_desc': 'Cada día estudias dos temas: teoría + práctica',
        'modes.feed_name': 'Feed',
        'modes.feed_infinite': 'modo infinito',
        'modes.feed_ai_desc': 'La IA sugiere temas, tú eliges qué estudiar',
        'modes.both_name': 'Ambos modos',
        'modes.marathon_activated': '¡Modo Maratón activado!',
        'modes.feed_activated': '¡Modo Feed activado!',
        'modes.your_settings': 'Tus ajustes',
        'modes.your_topics': 'Tus temas',
        'modes.time': 'Hora',
        'modes.reading_time': 'Tiempo de lectura',
        'modes.minutes': 'min',
        'modes.complexity': 'Complejidad',
        'modes.complexity_beginner': 'Principiante',
        'modes.complexity_basic': 'Básico',
        'modes.complexity_medium': 'Medio',
        'modes.complexity_advanced': 'Avanzado',
        'modes.complexity_expert': 'Experto',
        'modes.additional_reminder': 'Recordatorio adicional',
        'modes.feed_paused': 'Feed en pausa. Volver: /mode',
        'modes.marathon_paused': 'Maratón en pausa. Volver: /mode',
        # Estados de modos
        'modes.status_active': 'Activo',
        'modes.status_active_day': 'Activo (día {day}/{total}, {done} temas)',
        'modes.status_paused': 'En pausa',
        'modes.status_paused_day': 'En pausa (día {day}/{total})',
        'modes.status_not_started': 'No iniciado',
        'modes.status_not_started_f': 'No iniciada',
        'modes.status_completed': 'Completado',
        'modes.status_unknown': 'Estado desconocido',
        'modes.status_active_days': 'días activos: {days}',
        # Cadenas adicionales de modos
        'modes.error_occurred': 'Ocurrió un error. Por favor, intenta de nuevo.',
        'modes.use_learn': 'Usa /learn para continuar aprendiendo.',
        'modes.start_date': 'Fecha de inicio',
        'modes.start_date_not_set': 'no establecida',
        'modes.current': 'Actual',
        'modes.start_date_set': 'Fecha de inicio: {date}',
        'modes.reset_marathon_title': '¿Reiniciar maratón?',
        'modes.will_be_reset': 'Se reiniciará',
        'modes.topics_passed': 'temas completados',
        'modes.all_progress': 'Todo el progreso de días',
        'modes.feed_stats_kept': 'Las estadísticas del Feed se conservarán.',
        'modes.yes_reset': 'Sí, reiniciar',
        'modes.cancel': 'Cancelar',
        'modes.marathon_reset': '¡Maratón reiniciado!',
        'modes.new_start_date': 'Nueva fecha de inicio',
        'modes.use_learn_start': 'Usa /learn para comenzar a aprender.',
        'modes.reminders_title': 'Recordatorios',
        'modes.current_time': 'Actual',
        'modes.enter_time_format': 'Ingresa la hora en formato HH:MM',
        'modes.time_example': 'Ejemplo: `07:30` o `18:00`',
        'modes.two_reminders_hint': 'Para dos recordatorios, separa con coma:',
        'modes.two_reminders_example': 'Ejemplo: `07:00, 19:00`',
        'modes.invalid_time_format': 'Formato de hora inválido.',
        'modes.enter_time_example': 'Ingresa en formato HH:MM, ejemplo: `09:00` o `07:30, 19:00`',
        'modes.second_reminder_removed': 'Segundo recordatorio eliminado',
        'modes.time_set': 'Hora establecida: {time}',
        'modes.reminder_set': 'Recordatorio establecido: {time}',
        'modes.reminders_set': 'Recordatorios establecidos: {time1}, {time2}',
        'modes.complexity_title': 'Complejidad de preguntas',
        'modes.complexity_set': 'Complejidad: {level}',
        'modes.level_basic': 'Básico',
        'modes.level_basic_desc': 'comprensión de conceptos',
        'modes.level_medium': 'Medio',
        'modes.level_medium_desc': 'aplicación práctica',
        'modes.level_advanced': 'Avanzado',
        'modes.level_advanced_desc': 'análisis y síntesis',
        'modes.change_time': 'Cambiar hora',
        'modes.second': 'Segundo',
        'modes.add_second': 'Añadir segundo',
        'modes.select_time': 'Selecciona la hora',
        'modes.tomorrow': 'Mañana',
        'modes.day_after_tomorrow': 'Pasado mañana',
