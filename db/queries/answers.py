...
async def save_answer(pool, chat_id: int, topic_index: int, answer: str, 
...
async def get_answers(pool, chat_id: int, limit: int = 100) -> List[dict]:
...
async def get_weekly_work_products(pool, chat_id: int, week_offset: int = 0) -> List[dict]:
    """
...
async def get_answers_count_by_type(pool, chat_id: int) -> dict:
